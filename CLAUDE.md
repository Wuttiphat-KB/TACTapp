...
**งานที่ค้างฝั่ง App/Backend:** Charging History, Profile API, Build APK, capacity→kWh (รอขนาดถังน้ำมัน), gen Stop/status tag (รอทีม PLC — ดู `OPEN_QUESTIONS.md`)
...
ลำดับจริง (ใน `CP.py:1464-1475`, ต่างจาก §9.1) — เมื่อได้ RemoteStart:
`write_start_gen(1)` → `write_start_gen(0)` → `time.sleep(15)` → `remote_start_plc(1)` → `sleep(2)` → `remote_start_plc(0)`

**สถานะ gen tag แต่ละตัว:**

| tag | มีจริง? | หมายเหตุ |
|---|---|---|
| `Start_Gen` (start) | ✅ live | แต่ผูกอยู่ใน `if self.myremotestart_c1 == 1:` (`CP.py:1465`) → เรียกเดี่ยว (สั่งติดเครื่องเฉย ๆ ตอนยังไม่ชาร์จ) **ยังทำไม่ได้** — ตัวบล็อกปุ่ม AC |
| `Stop_Gen` (stop) | ❌ **ยังไม่มี** | grep ทั้ง tree = 0 hit — วันนี้สั่งติดเครื่องแต่ไม่เคยสั่งดับ → **ต้องขอทีม PLC** |
...
6.  EdgeBox loop: เห็น flag → HTTP pulse: remote_start_webvisit=1 → sleep(2s) → 0
7.  PLC: Start Generator (10–20s) → พร้อม → เริ่ม Charger
...
4. EdgeBox loop → HTTP pulse: remote_stop_webvisit=1 → sleep(3s) → 0
...
- [ ] Charging History
- [ ] `Stop_Gen` + `generator_status` GV tag (รอทีม PLC) + Timeout handling
